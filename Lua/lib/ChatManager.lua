...
function ChatGui:init(ws)
	...
	BC.Hooks:call("ChatGUI:Show", self)
end
...
function ChatManager:send_message(channel, sender, message)
	local args = BC.Hooks:call("ChatManager:SendMessage", channel, sender, message)
...
function ChatManager:_receive_message(channel, name, message, color, icon)
	local args = BC.Hooks:call("ChatManager:ReceiveMessage", channel, name, message, color, icon)
...
function ChatManager:receive_message_by_peer(channel, peer, message)
	local args = BC.Hooks:call("ChatManager:ReceivePeerMessage", channel, peer, message)
